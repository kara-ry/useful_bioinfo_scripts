    ...
    with pysam.VariantFile(vcf_file) as vcf:
        for record in vcf:
            ...
            if svtype not in variants:
                variants[svtype] = []
            variants[svtype].append((chromosome, position))
    return variants
...
def count_variants_and_genes(chromosome_lengths, gene_locations, variant_locations, window_size):
    ...
    for chromosome, length in chromosome_lengths.items():
        for window_start in range(1, length, window_size):
            ...
            row = [chromosome, window_start, window_end]
            for svtype, svtype_variants in variant_locations.items():
                variant_count = sum(window_start <= variant[1] <= window_end for variant in svtype_variants if variant[0] == chromosome)
                row.append(variant_count)
            gene_count = sum(window_start <= gene[1] <= window_end for gene in gene_locations if gene[0] == chromosome)
            row.append(gene_count)
            results.append(row)
    return results

def write_output(results, output_file, svtypes):
    header = "Chromosome\tStart\tEnd\t" + "\t".join(svtypes) + "\tGene_Count\n"
